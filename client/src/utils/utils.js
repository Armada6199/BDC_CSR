export default function calculateEMI(
    ...
    if(i<rates.length-1){
      if (loanAmount > 0) {
        ...
        if (loanAmount > totalMaxAfterDeduction) {
          ...
          if (loanAmount - totalMaxAfterDeduction > rates[i+1].min) {
  ...
  activeLoansDeductions.reduce(
    (accumulator, currentValue) => accumulator+Number(currentValue.activePayPerMonth),initialValue
  )
  const payPerMonth=EMI / numberOfMonths+activeLoansDeductions.reduce(
    (accumulator, currentValue) => accumulator+Number(currentValue.activePayPerMonth),initialValue
  )
  return {
    totalInterests,
